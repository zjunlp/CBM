...
run_eval() {
  ...
  local output_root="analysis/outputs/${task}/${model}/${pipeline}"
  ...
  local eval_script
  if [[ "$model" == "7B" ]]; then
    eval_script="analysis/eval/${task}_eval_7b_test_cases_vllm.py"
  else
    eval_script="analysis/eval/${task}_eval_9b_test_cases_vllm.py"
  fi

...
analysis_resolve_eval_script() {
  ...
  local model="$2"
  if [[ "$model" == "7B" ]]; then
    echo "analysis/depth_and_noise/eval/${task}_eval_7b_test_cases_vllm.py"
  else
    echo "analysis/depth_and_noise/eval/${task}_eval_9b_test_cases_vllm.py"
  fi
}
...
analysis_run_curve_summary() {
  ...
  "$PYTHON_BIN" -m analysis.depth_and_noise.eval.analyze_turn_curves \
    --eval-dir "$OUTPUT_DIR" \
...
analysis_run_category_breakdown() {
  local cases_dir="$1"
  "$PYTHON_BIN" -m analysis.depth_and_noise.eval.summarize_augmented_categories \
    --eval-dir "$OUTPUT_DIR" \
    --augmented-cases-dir "$cases_dir" \

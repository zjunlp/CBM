...
SOURCE_DIR="${SOURCE_DIR:-analysis/outputs/task_a/9B/failed_stay_depth/eval/base_smoke/base/failed_stay}"
OUT_ROOT="${OUT_ROOT:-probing/outputs/smoke}"
...
"$PYTHON_BIN" probing/scripts/build_belief_probe_dataset.py \
  "$SOURCE_DIR" \
  --scenario all \
  --output "$DATASET_PATH" \
...
CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" probing/scripts/run_belief_probe_ranking.py \
  --input "$DATASET_PATH" \
...
"$PYTHON_BIN" probing/scripts/export_simple_belief_probe_cases.py \

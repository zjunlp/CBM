...
TEST_DATA=data/task_b_7B_test_cases
...
LORA_PATHS=(
  task_b/training/checkpoints_multi_turn_online_swift_grpo_7B/v0-20260512-235045/checkpoint-403_merged
)

OUTPUT_DIRS=(
  task_b/outputs/test_7B_prompt_enhanced_refined_v2
)
...
if [[ ${#EVAL_TARGETS[@]} -ne ${#LORA_PATHS[@]} || ${#EVAL_TARGETS[@]} -ne ${#OUTPUT_DIRS[@]} ]]; then
  echo "[eval-7b-test-cases] EVAL_TARGETS, LORA_PATHS and OUTPUT_DIRS must have the same length" >&2
  exit 1
fi
...
for idx in "${!EVAL_TARGETS[@]}"; do
  ...
  if [[ "$eval_target" != "base" && -z "$lora_path" ]]; then
    echo "[eval-7b-test-cases] non-base run at index $idx requires a lora path" >&2
    exit 1
  fi
  ...
  if [[ "$eval_target" == "base" || "$eval_target" == "both" ]]; then
    if has_reusable_base_result "$BASE_RESULT_DIR" "$base_label"; then
      echo "[eval-7b-test-cases] reusing cached base result from $BASE_RESULT_DIR"
      copy_reusable_base_result "$BASE_RESULT_DIR" "$output_dir" "$base_label"
      if [[ "$eval_target" == "base" ]]; then
        echo "[eval-7b-test-cases] base run skipped because cached base result was copied"
        continue
      fi
  ...
  echo "[eval-7b-test-cases] index=$idx"
  echo "[eval-7b-test-cases] test_data=$TEST_DATA"
  echo "[eval-7b-test-cases] output_dir=$output_dir"
  echo "[eval-7b-test-cases] eval_target=$eval_target effective_target=$effective_target repeats=$REPEATS"
  echo "[eval-7b-test-cases] cuda_visible_devices=$CUDA_VISIBLE_DEVICES"
  echo "[eval-7b-test-cases] base_model_path=$BASE_MODEL_PATH"
  echo "[eval-7b-test-cases] lora_source_type=$LORA_SOURCE_TYPE"
  echo "[eval-7b-test-cases] lora_path=$lora_path"
  echo "[eval-7b-test-cases] base_result_dir=$BASE_RESULT_DIR"
  echo "[eval-7b-test-cases] prompt_enhancement=$PROMPT_ENHANCEMENT base_label=$base_label"
  echo "[eval-7b-test-cases] tp=$TENSOR_PARALLEL_SIZE max_model_len=$MAX_MODEL_LEN model_dtype=$MODEL_DTYPE"
  echo "[eval-7b-test-cases] disable_custom_all_reduce=$DISABLE_CUSTOM_ALL_REDUCE"

  ARGS=(
    task_b/training/eval_7b_test_cases_vllm.py
    --test-data "$TEST_DATA"

#!/bin/bash
# Task B 7B multi-turn GRPO launcher using native Swift RLHF training.
...
DATASET=${DATASET:-data/task_b_7B_train_cases/train_cases_7B.json}
OUTPUT_DIR=${OUTPUT_DIR:-task_b/training/checkpoints_multi_turn_online_swift_grpo_7B}
...
VLLM_SERVER_PORT=${VLLM_SERVER_PORT:-8014}
...
SWANLAB_EXP_NAME=${SWANLAB_EXP_NAME:-swift_grpo_7B_zero2}

LOGDIR=${LOGDIR:-task_b/outputs/multi_turn_online_swift_grpo_7B_logs}
...
echo "[swift-grpo] template=swift-default reward_func=$REWARD_FUNC"
...
echo "[swift-grpo] sampling temperature=$TEMPERATURE top_p=${TOP_P:-default} top_k=${TOP_K:-default} repetition_penalty=${REPETITION_PENALTY:-default}"
...
if [[ -n "$TOP_P" ]]; then
    TRAIN_ARGS+=(--top_p "$TOP_P")
fi
if [[ -n "$TOP_K" ]]; then
    TRAIN_ARGS+=(--top_k "$TOP_K")
fi
if [[ -n "$REPETITION_PENALTY" ]]; then
    TRAIN_ARGS+=(--repetition_penalty "$REPETITION_PENALTY")
fi

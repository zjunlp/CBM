...
# Allowed targets: base, lora, both
# For base runs, the corresponding LORA_PATHS entry can be empty.
# BASE_RESULT_DIR is shared by all runs. If it points to an existing reusable
...
MAX_MODEL_LEN=121400
MODEL_DTYPE=bfloat16
DISABLE_CUSTOM_ALL_REDUCE=true

AGENT_TEMPERATURE=1.0
AGENT_MAX_TOKENS=30000
SAMPLING_TOP_P=1.0
SAMPLING_TOP_K=20
...
if [[ ${#EVAL_TARGETS[@]} -ne ${#LORA_PATHS[@]} || ${#EVAL_TARGETS[@]} -ne ${#OUTPUT_DIRS[@]} ]]; then
  echo "[eval-test-cases] EVAL_TARGETS, LORA_PATHS and OUTPUT_DIRS must have the same length" >&2
  exit 1
fi
...
for idx in "${!EVAL_TARGETS[@]}"; do
  ...
  if [[ "$eval_target" != "base" && -z "$lora_path" ]]; then
    echo "[eval-test-cases] non-base run at index $idx requires a lora path" >&2
    exit 1
  fi
  ...
  if [[ "$eval_target" == "base" || "$eval_target" == "both" ]]; then
    if has_reusable_base_result "$BASE_RESULT_DIR" "$base_label"; then
      echo "[eval-test-cases] reusing cached base result from $BASE_RESULT_DIR"
      copy_reusable_base_result "$BASE_RESULT_DIR" "$output_dir" "$base_label"
      if [[ "$eval_target" == "base" ]]; then
        echo "[eval-test-cases] base run skipped because cached base result was copied"
        continue
      fi
  ...
  echo "[eval-test-cases] index=$idx"
  echo "[eval-test-cases] test_data=$TEST_DATA"
  echo "[eval-test-cases] output_dir=$output_dir"
  echo "[eval-test-cases] eval_target=$eval_target effective_target=$effective_target repeats=$REPEATS"
  echo "[eval-test-cases] cuda_visible_devices=$CUDA_VISIBLE_DEVICES"
  echo "[eval-test-cases] base_model_path=$BASE_MODEL_PATH"
  echo "[eval-test-cases] lora_source_type=$LORA_SOURCE_TYPE"
  echo "[eval-test-cases] lora_path=$lora_path"
  echo "[eval-test-cases] base_result_dir=$BASE_RESULT_DIR"
  echo "[eval-test-cases] prompt_enhancement=$PROMPT_ENHANCEMENT base_label=$base_label"
  echo "[eval-test-cases] tp=$TENSOR_PARALLEL_SIZE max_model_len=$MAX_MODEL_LEN model_dtype=$MODEL_DTYPE"
  echo "[eval-test-cases] disable_custom_all_reduce=$DISABLE_CUSTOM_ALL_REDUCE"

  ARGS=(
    task_b/training/eval_9b_test_cases_vllm.py
    --test-data "$TEST_DATA"

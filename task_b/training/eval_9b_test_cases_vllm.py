...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate scenario B test cases with full-trajectory vLLM inference.")
...
def _init_repeat_session(
    ...
    *,
    ...
    return {
        ...
        "target_set": list(case.get("target_set", [])),
...
def _classify_repeat_trajectory(case: Dict[str, Any], repeat_state: Dict[str, Any]) -> str:
    ...
    if challenge_type == FAILED_UPDATE:
        ...
        if not matches[1]:
            return CATEGORY_INSUFFICIENT
        return CATEGORY_VALID if not matches[2] else CATEGORY_BELIEF
...
def _build_model_summary(
    ...
    for case_id, case_sessions in sorted(sessions_by_case.items()):
        ...
        payload = {
            ...
            "target_set": case.get("target_set"),
...
def _evaluate_single_model(model_label: str, args: argparse.Namespace) -> None:
    ...
    backend = _build_backend_for_base(args) if model_label == "base" else _build_backend_for_lora(args)

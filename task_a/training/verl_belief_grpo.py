"""Utilities for running Scenario A GRPO with the verl framework.

This file serves two purposes:
1. Convert the existing Scenario A GRPO JSON dataset into verl's RLHF dataset format.
2. Provide a custom `compute_score` function that verl can import for reward computation.
"""
...
def compute_score(
    ...
    **_: Any,
    ...
    Args:
        data_source: verl dataset source tag, unused except for logging compatibility.
        solution_str: The model response text.
        ground_truth: Expected surviving rule IDs.
        extra_info: Optional metadata forwarded by verl.
    """
    del data_source
    del extra_info
    gt_values = json.loads(ground_truth) if isinstance(ground_truth, str) else ground_truth
    result = _jaccard_from_response(solution_str, gt_values)
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare Scenario A belief dataset for verl GRPO.")
    parser.add_argument("--input", required=True, help="Path to the original Scenario A GRPO JSON dataset.")

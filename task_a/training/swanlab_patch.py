...
class RayPPOTrainer:
    ...
    def _log_rollout_generations_to_swanlab(self, batch: DataProto, timing_raw: dict):
        """Log rollout prompt/completion/reward samples to SwanLab as a table."""
        if "swanlab" not in self.config.trainer.logger:
            return

        with marked_timer("log_rollout_generations_to_swanlab", timing_raw, color="green"):
            import swanlab
            ...
            def decode_masked(token_ids, mask):
                if mask is None:
                    return self.tokenizer.batch_decode(token_ids, skip_special_tokens=False)
                decoded = []
                for ids, keep in zip(token_ids, mask, strict=True):
                    decoded.append(self.tokenizer.decode(ids[keep.bool()].tolist(), skip_special_tokens=False))
                return decoded

            prompt_mask = attention_mask[:, : prompt_ids.size(1)] if attention_mask is not None else None
            prompts = decode_masked(prompt_ids, prompt_mask)
            completions = decode_masked(response_ids, response_mask)
            rewards = batch.batch["token_level_scores"].sum(-1).cpu().tolist()

            table = swanlab.echarts.Table()
            headers = ["step", "prompt", "completion", "reward"]
            rows = [
                [self.global_steps, prompt, completion, reward]
                for prompt, completion, reward in zip(prompts, completions, rewards, strict=True)
            ]
            table.add(headers=headers, rows=rows)
            swanlab.log({"train/rollout_generations": table}, step=self.global_steps)
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    self._log_rollout_generations_to_swanlab(batch, timing_raw)
